...
public class SafeArea : MonoBehaviour
{
    ...
    [SerializeField] private bool ignoreBottom = true;
    ...
    void ApplySafeArea(Rect r)
    {
        ...
        if (r.y == 0 && r.height == Screen.height)
        {
            if (logging)
            {
                Debug.Log("No need for safe area adjustment, full screen without notch.");
            }
        ...
        if (ignoreBottom)
        {
            float bottomOffset = Screen.height * 0.05f; // Alt kısım için offset
            r.y = 0;
            r.height = Screen.height - bottomOffset;
        }
